...
- **Editor-Umbau: Website-Struktur spiegeln (2026-06-03):** Das linke Editor-Pane war flach (eine lange Karten-Liste). Jetzt spiegelt es die Website-Struktur, **bleibt aber bewusst „leicht"** — alle Karten bleiben gemountet (kein echtes Seitenbaum-CMS, das Karten ent-/mountet würde — das bräche den Scroll-Spy, Advisor-Erkenntnis). Konkret: (1) Sticky-Navi `.editor-nav` oben im Pane (Pillen, gespiegelt von der Site-Header-Navi: „Startseite" + je Kategorie ein `navLabel`), die per `scrollToGroup()` zum Gruppenanker scrollt. (2) Die 6 Home-Karten sind in `<div id="grp-home" className="editor-group">` mit Überschrift „Startseite" gewickelt; die Kategorien in eine eigene `editor-group`. (3) Jede Kategorie-Karte trägt `id="grp-cat-<id>"` (in `CategoryTreeEditor`) als Navi-Ziel. Anker haben `scroll-margin-top: 120px`. **Sync intakt** verifiziert: 6 `data-card`, 4 `data-block-key`, Home-Karten-Sprung landet Kontakt exakt auf `SPY_LINE` (110) + hebt Kontakt hervor; Navi-Pillen scrollen das Editor-Pane. **Bekannte Rauheit:** Kategorie-Navi-Sprung landet locker bei ~220 px statt ~120 (tunebar über `scroll-margin-top`, nicht behoben). Die Navi-Pillen treiben **nur** das Editor-Pane, noch nicht die Vorschau (offene Ausbaustufe).

- **Rechtsseiten editierbar (2026-06-05):** Impressum und Datenschutz sind keine statischen Platzhalter mehr, sondern CMS-Inhalt. Datenmodell: `legal: { impressum, datenschutz }` mit je `{ title, sections: [{heading, body}] }` in `lib/default-content.ts` (Body: Leerzeile trennt Absätze, `\n` bleibt via CSS `white-space: pre-line` erhalten — kein Markdown-Parser). Geteilter Renderer `app/components/views/LegalPageView.tsx`; `app/impressum/page.tsx` und `app/datenschutz/page.tsx` sind jetzt async Server-Components, die `getContent().legal` lesen. **Blocking-Fix:** bestehende `content`/`draft`-Zeilen kennen `legal` nicht (`migrate.mjs` = ON CONFLICT DO NOTHING, seedet nur leere Tabelle). Darum Lese-Normalisierung `withDefaults()` in `lib/content.ts` (`legal: value.legal ?? DEFAULT_CONTENT.legal`) — KEINE Einmal-Migration (Prod hat dieselben veralteten Zeilen). Editor: eigene, **immer sichtbare** Gruppe „Rechtliches" am Ende des Formulars (bewusster kleiner Bruch mit „Editor folgt Vorschau" — Rechtsseiten werden in der Vorschau nicht gerendert, also kein Auslöser). `LegalEditor`-Komponente (2×) mit Titel + Abschnitten (Überschrift/Text), Add/Remove je Abschnitt (Remove mit Bestätigung). Im Browser end-to-end verifiziert gegen die BESTEHENDE Zeile (nicht reseed): Fallback rendert Defaults, edit→Speichern→Veröffentlichen ändert die öffentliche Seite, Abschnitt hinzufügen/entfernen funktioniert. **Bewusst nicht gebaut:** Live-Vorschau der Rechtsseiten (große Plumbing, geringer Mehrwert für Fließtext) und Abschnitts-Reorder — beides mögliche Folge-Slices. `metadata.title` bleibt hardcodiert (folgt einer Admin-Umbenennung des H1 nicht).
...
3. ~~**Admin-Editor strukturieren**~~ ✅ erledigt 2026-06-03 (s. „Editor-Umbau" oben). Offene Folge-Ausbaustufen: Navi-Pillen auch die Vorschau treiben; einklappbare Gruppen + Unterseiten-Sublinks in der Navi; Kategorie-Navi-Landung straffen (~220 → ~120 via `scroll-margin-top`).
