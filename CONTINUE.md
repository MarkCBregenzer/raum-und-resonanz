...
- **Scroll-Spy (2026-06-03):** Bisher hob sich die aktive Sektion/der aktive Baustein nur beim KLICKEN hervor. Jetzt wandert die Hervorhebung automatisch mit, während Mark durch die Vorschau scrollt. Ein `IntersectionObserver` im Iframe meldet das jeweils oberste sichtbare Ziel (in einem schmalen Aktivierungsband nahe oben, `rootMargin: "-80px 0px -70% 0px"`) per `postMessage` an den Editor — dieselben Nachrichten wie der Klick-Sync (`MSG_ACTIVE_SECTION`/`MSG_ACTIVE_BLOCK`), zusätzlich `spy: true`. Der Editor scrollt die Karte beim Spy nur sanft bei Bedarf ins Bild (`block: "nearest"`, sofort) statt mittig-smooth (Klick), sonst würde das Panel zucken. **Zwei Fallen entschärft** (vom Advisor benannt, beide verifiziert): (1) Observer wird NUR neu gebaut, wenn sich die Ziel-MENGE ändert (Dep `[pathname, spyTargetCount]`, nicht der ganze `content`), und der Dedupe-Wächter steht in einem `useRef` — sonst würde jeder Tastendruck den Observer neu aufsetzen und den Editor anstupsen (verifiziert: 0 Spy-Meldungen beim Tippen). (2) Ein gezielter Sprung darf nicht sofort vom Spy überschrieben werden: landet das Ziel mitten im Band, ragt der Vorgänger oben hinein und würde als „oberstes" die Hervorhebung klauen. Lösung: nach einem Sprung pausiert der Spy (`jumpHoldRef`), bis Mark WIRKLICH selbst scrollt (Wheel/Touch/Tastatur) — ein simpler Timer reicht nicht, weil späte Layout-Shifts (Bild/Font lädt nach) den Observer Sekunden später nochmal feuern lassen. Beide Richtungen + beide Fallen im Browser verifiziert (Startseite und Unterseite). Nur zwei Dateien berührt: `PreviewClient.tsx` (Observer + Hold) und `AdminEditor.tsx` (spy-Flag → sanftes Scrollen).
